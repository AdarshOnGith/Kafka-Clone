# Run All E2E Tests
# Orchestrates execution of all end-to-end tests

Write-Host "🚀 Starting Complete E2E Test Suite..." -ForegroundColor Green
...
$testScripts = @(
    # Setup tests
    @{
        Path = "$baseDir\scripts\setup\01-setup-services.ps1"
        Name = "Service Setup"
        Description = "Configure service mappings and validate prerequisites"
    },
    @{
        Path = "$baseDir\scripts\setup\02-start-services.ps1"
        Name = "Service Startup"
        Description = "Parallel service startup with health validation"
    },
    @{
        Path = "$baseDir\scripts\setup\03-init-test-data.ps1"
        Name = "Test Data Initialization"
        Description = "Initialize brokers and topics for testing"
    },
    ...
    @{
        Path = "$baseDir\scripts\storage\consumer-flow-test.ps1"
...
$exportPath = "$baseDir\test-suite-results.json"
$exportData = @{
